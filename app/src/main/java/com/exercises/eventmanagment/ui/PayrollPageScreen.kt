...
@Composable
fun PayrollPageScreen(
    navController: NavController,
    innerPadding: PaddingValues,
    viewModel: PayrollPageViewModel = koinViewModel(),
    isDarkTheme: Boolean = false
) {

    val payrollScreenUiState by viewModel.uiState.collectAsStateWithLifecycle()
    val colorScheme = if (isDarkTheme) DarkColorScheme else LightColorScheme

    MaterialTheme(colorScheme = colorScheme) {
        Log.d(TAG, "PayrollPageScreen() -> composed / recomposed")

        when (val uiState = payrollScreenUiState) {
            is PayrollScreenUiState.Loading ->
                PayrollPageScreenLoading()

            is PayrollScreenUiState.Error -> {
                PayrollPageScreenError(
                    msg = uiState.errorMassage!!,
                    onRetry = {}
                )
            }

            is PayrollScreenUiState.Ready -> {
...
@Composable
fun PayrollPageScreenLoading(modifier: Modifier = Modifier) {
...
@Composable
fun PayrollPageScreenError(msg: String, onRetry: () -> Unit, modifier: Modifier = Modifier) {
...
@Composable
fun PayrollPageScreenReady(
    navController: NavController,
    uiState: PayrollScreenUiState.Ready,
    innerPadding: PaddingValues,
) {
    ...
    val onChangeScreenClick: () -> Unit = {
        ...
        val changeScreen = Screens.PayrollAddPageScreen.route
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(Screens.PayrollPageScreen.title) }
            )
        },
